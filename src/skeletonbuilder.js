...
export default class SkeletonBuilder {
  ...
  static Build (polygon, holes = null) {
    ...
    while (!queue.Empty) {
      ...
      for (const event of this.LoadAndGroupLevelEvents(queue)) {
        if (event.IsObsolete) { continue }

        if (event instanceof EdgeEvent) { throw new Error('All edge@events should be converted to MultiEdgeEvents for given level') }
        if (event instanceof SplitEvent) { throw new Error('All split events should be converted to MultiSplitEvents for given level') }
        if (event instanceof MultiSplitEvent) { this.MultiSplitEvent(event, sLav, queue, edges) } else if (event instanceof PickEvent) { this.PickEvent(event) } else if (event instanceof MultiEdgeEvent) { this.MultiEdgeEvent(event, queue, edges) } else { throw new Error('Unknown event type: ' + event.GetType()) }
      }
  ...
  static InitPolygon (polygon) {
    if (polygon === null) { throw new Error('polygon can\'t be null') }

    if (polygon[0].Equals(polygon[polygon.Count - 1])) { throw new Error('polygon can\'t start and end with the same point') }
  ...
  static MultiSplitEvent (event, sLav, queue, edges) {
    ...
    for (let i = 0; i < edgeListSize; i++) {
      ...
      if (LavUtil.IsSameLav(beginNextVertex, endPreviousVertex)) {
        ...
        for (const vertex of lavPart) { lav.AddLast(vertex) }
      } else {
    ...
    for (let i = 0; i < edgeListSize; i++) {
      ...
      if (chainBegin.CurrentVertex !== null) { chainBegin.CurrentVertex.IsProcessed = true }
      if (chainEnd.CurrentVertex !== null) { chainEnd.CurrentVertex.IsProcessed = true }
    }
  }
  ...
  static CorrectBisectorDirection (bisector, beginNextVertex, endPreviousVertex, beginEdge, endEdge) {
    ...
    if (beginEdge !== beginEdge2 || endEdge !== endEdge2) { throw new Error() }
    ...
    if (beginEdge.Norm.Dot(endEdge.Norm) < -0.97) {
      ...
      if (bisector.U.Dot(bisectorPrediction) < 0) { bisector.U.Negate() }
    }
  }
  ...
  static AddSplitFaces (lastFaceNode, chainBegin, chainEnd, newVertex) {
    if (chainBegin instanceof SingleEdgeChain) {
      if (lastFaceNode === null) {
        ...
        if (newVertex.RightFace !== null) { throw new Error('newVertex.RightFace should be null') }
    ...
    if (chainEnd instanceof SingleEdgeChain) {
      if (lastFaceNode === null) {
        ...
        if (newVertex.LeftFace !== null) { throw new Error('newVertex.LeftFace should be null.') }
  ...
  static CreateOppositeEdgeChains (sLav, chains, center) {
    ...
    for (const chain of chains) {
      if (chain instanceof SplitChain) {
        ...
        if (oppositeEdge !== null && !oppositeEdges.Contains(oppositeEdge)) {
          ...
          if (nextVertex !== null) { oppositeEdgeChains.Add(new SingleEdgeChain(oppositeEdge, nextVertex)) } else {
    ...
    for (const chain of chainsForRemoval) { chains.Remove(chain) }
  ...
  static RemoveEventsUnderHeight (queue, levelHeight) {
    while (!queue.Empty) {
      if (queue.Peek().Distance > levelHeight + this.SplitEpsilon) { break }
      queue.Next()
    }
  }
  ...
  static IsEventInGroup (parentGroup, event) {
    if (event instanceof SplitEvent) { return parentGroup.Contains((event).Parent) }
  ...
  static AddEventToGroup (parentGroup, event) {
    if (event instanceof SplitEvent) { parentGroup.Add((event).Parent) } else if (event instanceof EdgeEvent) {
  ...
  static CreateLevelEvent (eventCenter, distance, eventCluster) {
    ...
    if (chains.Count === 1) {
      ...
      if (chain.ChainType === ChainType.ClosedEdge) { return new PickEvent(eventCenter, distance, chain) }
      if (chain.ChainType === ChainType.Edge) { return new MultiEdgeEvent(eventCenter, distance, chain) }
      if (chain.ChainType === ChainType.Split) { return new MultiSplitEvent(eventCenter, distance, chains) }
    }

    if (chains.Any(chain => chain.ChainType === ChainType.ClosedEdge)) { throw new Error('Found closed chain of events for single point, but found more then one chain') }
  ...
  static LoadLevelEvents (queue) {
    ...
    if (levelStart === null || levelStart.IsObsolete) { return level }
    ...
    Math.abs(event.Distance - levelStartHeight) < this.SplitEpsilon) {
      ...
      if (!nextLevelEvent.IsObsolete) { level.Add(nextLevelEvent) }
    }
    return level
  }
  ...
  static AssertMaxNumberOfInteraction (count) {
    count++
    if (count > 10000) { throw new Error('Too many interaction: bug?') }
    return count
  }
  ...
  static MakeClockwise (holes) {
    ...
    for (const hole of holes) {
      if (PrimitiveUtils.IsClockwisePolygon(hole)) { ret.Add(hole) } else {
        hole.Reverse()
        ret.Add(hole)
      }
    }
    ...
    for (const face of faces) {
      if (face.Size > 0) {
        ...
        for (const fn of face.Iterate()) {
          ...
          if (!distances.ContainsKey(point)) { distances.Add(point, fn.Vertex.Distance) }
        }
  ...
  static InitEvents (sLav, queue, edges) {
    for (const lav of sLav) {
      for (const vertex of lav.Iterate()) { this.ComputeSplitEvents(vertex, edges, queue, -1) }
    }
  ...
  static ComputeCloserEdgeEvent (vertex, queue) {
    ...
    if (point1.Equals(Vector2d.Empty) && point2.Equals(Vector2d.Empty)) { return -1 }

    let distance1 = Number.MAX_VALUE
    let distance2 = Number.MAX_VALUE

    if (point1.NotEquals(Vector2d.Empty)) { distance1 = point.DistanceSquared(point1) }
    if (point2.NotEquals(Vector2d.Empty)) { distance2 = point.DistanceSquared(point2) }

    if (Math.abs(distance1 - this.SplitEpsilon) < distance2) { queue.Add(this.CreateEdgeEvent(point1, vertex, nextVertex)) }
    if (Math.abs(distance2 - this.SplitEpsilon) < distance1) { queue.Add(this.CreateEdgeEvent(point2, previousVertex, vertex)) }
  ...
  static ComputeEdgeEvents (previousVertex, nextVertex, queue) {
    ...
    if (point.NotEquals(Vector2d.Empty)) { queue.Add(this.CreateEdgeEvent(point, previousVertex, nextVertex)) }
  }
  ...
  static calcCandidatePointForSplit (vertex, edge) {
    ...
    if (edgesCollide.Equals(Vector2d.Empty)) { throw new Error('Ups this should not happen') }
    ...
    if (candidatePoint.Equals(Vector2d.Empty)) { return null }
    ...
    if (edge.BisectorPrevious.IsOnRightSite(candidatePoint, this.SplitEpsilon) &&
      ...
      if (edge.BisectorPrevious.IsOnLeftSite(candidatePoint, this.SplitEpsilon)) { return new SplitCandidate(candidatePoint, distance, null, edge.Begin) }
      if (edge.BisectorNext.IsOnRightSite(candidatePoint, this.SplitEpsilon)) { return new SplitCandidate(candidatePoint, distance, null, edge.Begin) }
  ...
  static ChoseLessParallelVertexEdge (vertex, edge) {
    ...
    if (edgeADot + edgeBDot >= 2 - this.SplitEpsilon) { return null }

    if (edgeADot > edgeBDot) { vertexEdge = edgeB }
  ...
  static ComputeIntersectionBisectors (vertexPrevious, vertexNext) {
    ...
    if (vertexPrevious.Point.Equals(intersect) || vertexNext.Point.Equals(intersect)) { return Vector2d.Empty }
  ...
  static ChooseOppositeEdgeLav (edgeLavs, oppositeEdge, center) {
    if (!edgeLavs.Any()) { return null }

    if (edgeLavs.Count === 1) { return edgeLavs[0] }
    ...
    for (const end of edgeLavs) {
      ...
      if (PrimitiveUtils.IsPointInsidePolygon(center, points)) { return end }
    }
  ...
  static FindEdgeLavs (sLav, oppositeEdge, skippedLav) {
    ...
    for (const lav of sLav) {
      if (lav === skippedLav) { continue }

      const vertexInLav = this.GetEdgeInLav(lav, oppositeEdge)
      if (vertexInLav !== null) { edgeLavs.Add(vertexInLav) }
    }
    return edgeLavs
  }
  ...
  static GetEdgeInLav (lav, oppositeEdge) {
    for (const node of lav.Iterate()) {
      if (oppositeEdge === node.PreviousEdge ||
        oppositeEdge === node.Previous.Next) { return node }
    }
